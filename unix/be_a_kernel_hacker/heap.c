...
void heap_pop(struct heap *h){
	if(h->cur_size > 0 ){
		char * tmp = h->data[0];
		h->data[0] = h->data[h->cur_size];
		h->data[h->cur_size] = tmp;
		--h->cur_size;
	}
}
...
static ssize_t print_heap(struct heap * h, char __user * out,
...
static struct heap * heap_alloc(unsigned long element_size){
	...
	h->data = kzalloc(h->max_size +1 , GFP_KERNEL);
	if (unlikely(!h->data)){
		kfree(h);
		return NULL;
	}
	for(int i=0; i< h->max_size +1; ++i){
		h->data[i] = kzalloc(_element_size, GFP_KERNEL);
